...
namespace JNHub.JN
{
    class JNItem
    {
        ...
        public string FormattedSmallDescription
        {
            get
            {
                var tryToSplitStringFirst = Description.Trim().Split('\n')[0];
                var maxLength = 200;
                if (tryToSplitStringFirst.Length < maxLength)
                {
                    maxLength = tryToSplitStringFirst.Length;
                }
        ...
        public BitmapImage Image
        {
            get
            {
                if (this.ImageURL != null)
                    return new BitmapImage(new Uri(this.ImageURL, UriKind.Absolute));
                else
        ...
        public Windows.UI.Xaml.Controls.Symbol ItemTypeSymbol
        {
            get
            {
                if (this.isPodcast)
                    return Windows.UI.Xaml.Controls.Symbol.Microphone;

                if (this.isVideo)
                    return Windows.UI.Xaml.Controls.Symbol.Video;

                return Windows.UI.Xaml.Controls.Symbol.PreviewLink;
            }
        }
    }
}
